...
1. ZIP ファイルを SC-401-CL1 の**デスクトップ**に解凍します。 **DeviceComplianceLocalOnboardingScript.cmd** という名前のスクリプトがあるはずです。
...
1. **[アクション]** で、**[+ アクションの追加]**、**[デバイスでのアクティビティの監査または制限]** を選択します。

1. 新しい **[デバイスでのアクティビティの監査または制限]** セクション:
    - **[すべてのアプリのファイル アクティビティ]** セクションで、**[リムーバブル USB デバイスにコピー]** が選択されていることを確認します。
    - **[リムーバブル USB デバイスにコピー]** の左側にあるドロップダウンを選択して、アクションを **[監査のみ]** から **[ブロック]** に変更します。

1. **[ユーザー通知]**:
    - **[ユーザーへのお知らせに通知を使い、機密情報の適切な使用についてのユーザーの教育に役立てる]** の切り替えを**オン**にします。
    - **[アクティビティが制限されている場合にユーザーにポリシー ヒント通知を表示する]** のチェックボックスをオンにします。
...
1. **[ポリシー モード]** ページで、**[シミュレーション モードでポリシーを実行する]** を選択します。
   ...
   - また、**[シミュレーションから 15 日以内に編集されていない場合は、ポリシーをオンにする]** チェック ボックスをオンにします。
...
1. 引き続き Client 1 VM (SC-401-CL1) に **SC-401-cl1\admin** アカウントでログインし、Microsoft 365 には **Joni Sherman** としてログインしている必要があります。
...
このタスクでは、Google Chrome に Microsoft Purview 拡張機能をインストールして、サポートされているブラウザーでエンドポイント DLP ポリシーの動作をテストします。
