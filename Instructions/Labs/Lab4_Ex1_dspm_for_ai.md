...
1. (省略可能) ラベル付きコンテンツに対する Copilot のアクセスをブロックする
...
1. AI 用 DSPM で作成されたルールの構成を確認します。
   ...
   - **[アクション]** で、**[サービス ドメインとブラウザー アクティビティ]** が **[生成 AI Web サイト]** の **[オーバーライドによるブロック]** に設定されていることを確認します。
...
## タスク 3: (任意) ラベルが設定されたコンテンツへの Copilot のアクセスをブロックする

Copilot が秘密度ラベルで保護されたコンテンツを使用して処理または応答できないようにすることで、リスクをさらに軽減できます。

1. Microsoft Purview で、**[ソリューション]**、**[AI 用 DSPM]**、**[推奨事項]** の順に選択して **[AI 用 DSPM]** に移動します。

1. **[Microsoft 365 Copilot で参照されている機密データを保護する (プレビュー)]** という推奨事項を選択します。
...
1. **[Microsoft 365 Copilot で参照されている機密データを保護する (プレビュー)]** という推奨事項を選択し、**[完了としてマーク]** を選択します。

ラベルが設定されたコンテンツが Copilot のプロンプトと応答で使用されないようにする DLP ポリシーが作成されました。

## タスク 4: データ評価を実行してラベルが設定されていないコンテンツを検出する

ラベル設定範囲の潜在的なギャップを把握するために、データ評価を実行して、Copilot によってアクセスされる可能性のあるファイルで秘密度ラベルの無いものを特定します。

1. **[AI 用 DSPM]** で、**[Copilot 応答で参照されている機密データを保護する]** という推奨事項を選択します。

1. **[Copilot 応答で参照されている機密データを保護する]** ペインで概要を確認し、**[評価に移動]** を選択します。

1. **[データ評価 (プレビュー)]** ページで、**[評価の作成 (プレビュー)]** を選択します。
