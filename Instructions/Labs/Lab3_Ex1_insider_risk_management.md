...
1. **[ポリシー インジケーター]** ページで、 **すべて選択** を展開して選択し、次のカテゴリのすべてのインジケーターを有効にします。
...
1. **[ポリシー]** ページで、**[ポリシーの作成]** を選択してから、**[クリック ポリシー]** を選択します。

1. **[クイック ポリシーの作成]** ポップアップで、**[データの漏洩]** から **[開始する]** を選択します。

1. データ漏洩のクイック ポリシーを作成するための設定を確認し、**[ポリシーを作成する]** を選択します。

1. **[データ漏洩のポリシーを作成しています]** ページで、次のチェック ボックスをオンにします。

   - ポリシーに未解決の警告があるときにメールを受信する
   - 重大度の高い新しいアラートが生成されたときにメールを受信する

     次に、**[通知設定の更新]** を選択します。

1. **[データ漏洩のポリシーを作成しています]** ページの下部にある **[完了]** を選択します。
...
1. データの漏洩は、主にデータ流出のインサイダー リスク ポリシーです。 シーケンス インジケーターを有効にするダイアログで、**[すべて選択]** を選択して、必要なすべての **[流出インジケーター]** を有効にし、**[保存]** を選択します。

1. **[このポリシーのトリガー イベントの選択]** ページで、**[次へ]** を選択します。

1. **[イベントをトリガーするしきい値の選択]** ページで、**[次へ]** を選択します。

1. **[インジケーター]** ページで、**[次へ]** を選択します。
...
1. 下にスクロールし、トグルを選択して **[オン]** に切り替え、**[Microsoft コンプライアンス センターとエンドポイント アラートを共有する]** を有効にします。
...
1. **[ポリシー インジケーター]** ページで、 **すべて選択** を展開して選択し、次のカテゴリのすべてのインジケーターを有効にします。
   ...
   - Microsoft Defender for Endpoint のインジケーター (プレビュー)
   - 危険な閲覧インジケーター (プレビュー)

1. ページの下部にある **[保存]** を選択します。

1. **[優先度ユーザー グループ]** タブを選択した後、**[+ 優先度ユーザー グループの作成]** を選択します。
...
1. ポップアップで、**[インサイダー リスク管理]** のチェック ボックスをオンにし、**[追加]** を選択します。
...
1. **[ポリシー]** ページで、**[ポリシーの作成]** を選択してから、**[カスタム ポリシー]** を選択します。

1. **[ポリシー テンプレートの選択]** ページで、**[優先度の高いユーザーによるセキュリティ ポリシー違反 (プレビュー)]** を選択し、[次へ] を選択します。
...
1. **[ユーザー、グループ、アダプティブ スコープの選択]** ページで、**[優先ユーザー グループの追加または編集]** を選択します。

1. **[優先ユーザー グループの選択]** ポップアップで、**[ファイナンス チーム]** グループのチェック ボックスをオンにし、[追加] を選択します。
